...
T_MAX = 1024
from torch.utils.cpp_extension import load
wkv_cuda = load(name="wkv", sources=["cuda/wkv_op%s.cpp"%("2"*(CUDA_KERNEL_VERSION==4)), f"cuda/wkv_cuda_v{CUDA_KERNEL_VERSION}.cu"],
 verbose=True, extra_cuda_cflags=['-res-usage', '--maxrregcount 60', '--use_fast_math', '-O3', '-Xptxas -O3', f'-DTmax={T_MAX}'])


# state = batched torch.stack((aa, bb, pp), dim=-1) where aa,bb,pp are as in model_run's RNN mode
...
class WKV_part(torch.autograd.Function):
    ...
    def forward(ctx, B, T, C, w, u, k, v, last_state):
        ...
        wkv_cuda.forward2(B, T, C, w, u, k, v, last_state, y, new_state)
        return y, new_state
    ...
    def backward(ctx, gy, gnew_state):
        ...
        wkv_cuda.backward2(B, T, C, w, u, k, v, last_state, gy.contiguous(), gnew_state.contiguous(), gw, gu, gk, gv, glast_state)
