...
class TRIE_TOKENIZER():
    ...
    def encodeStrWithOffset(self,src:str,max_length=None):
        ...
        while (idx < len(src) and (max_length is None or len(tokens)<max_length)):
            ...
            _, token = next(iter(values))            
            tokens.append(token)
            offsets.append((offset_mapping[_idx], offset_mapping[idx] if idx < len(src) else offset_mapping[idx-1]+1))
...
THREEBYTES = 0x800
FOURBYTES = 0x10000
def get_utf8_bytes_offset(text):
    byte_offset_mapping = []
    for offset, character in enumerate(text):
        codepoint = ord(character)
        if codepoint < TWOBYTES:
            byte_offset_mapping += [offset]
        elif codepoint < THREEBYTES:
            byte_offset_mapping += [offset]*2
        elif codepoint < FOURBYTES:
            byte_offset_mapping += [offset]*3
        else:
            byte_offset_mapping += [offset]*4
